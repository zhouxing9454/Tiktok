...
func UserLogin(name string, password string) (*LoginAndRegisterResponse, int, error) {
	...
	hashPassword := userDao.GetPassword(name)
	verifyPassword := utils.HashPassword(password, strings.Split(hashPassword, ":")[1])
	password = verifyPassword + ":" + strings.Split(hashPassword, ":")[1]
